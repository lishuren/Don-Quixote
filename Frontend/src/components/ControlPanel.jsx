...
/**
 * ControlPanel component - table and robot service controls
 * @param {Object} props - Component props
 * @param {Function} props.onStartDelivery - Callback to start delivery to a table
 * @param {boolean} props.isRunning - Whether simulation is running
 * @param {number|null} props.targetTable - Current target table
 * @param {string|null} props.currentAction - Current action type
 * @param {boolean} props.showMap - Whether the map is currently displayed
 */
function ControlPanel({
  ...
  const { tableCount, robotCount } = useSelector((state) => state);
    ...
    <VegaCard padding="size-16" style={{ height: "100%" }}>
      <VegaFlex direction="col" gap="size-12" style={{ height: "100%" }}>
        ...
        <VegaFlex direction={"col"} gap={"size-12"} style={{ height: "70%" }}>
          <VegaFont variant="font-h6">Table Service <VegaFont variant={"font-field-label-xs-em"}>Total ({tables.length}):</VegaFont></VegaFont>
                ...
                >
                  ...
                  >
                    <VegaFlex gap={"size-8"} alignItems="center">
                      ...
                      />
                      <VegaFont variant="font-p2-short">
                        Table {tableNum}
                      </VegaFont>
                    </VegaFlex>
        ...
        <VegaFlex direction={"col"} gap={"size-12"} style={{height: '19%'}}>
          <VegaFont variant="font-h6">Robot Service <VegaFont variant={"font-field-label-xs-em"}>Total ({robots.length}):</VegaFont></VegaFont>
