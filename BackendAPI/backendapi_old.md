...
|---------|--------|-------------|
| `autoMergeEnabled` | `true`/`false` | Auto-merge overlapping zones on creation |
| `autoMergeThresholdPercent` | `0-100` | Merge if overlap exceeds this % (0 = any overlap) |
...
| `typeConflictStrategy` | `USE_HIGHEST_SEVERITY`, `USE_FIRST`, `REQUIRE_SAME` | How to resolve type conflicts |

*   **Type Severity Order:** `EMERGENCY` > `WET_FLOOR` > `MAINTENANCE` > `CONSTRUCTION` > `RESERVED`

*   **Auto-Merge SignalR Event:**
    ```json
    {
      "event": "ZonesAutoMerged",
      "payload": {
        "mergedZoneId": "zone_101",
        "originalZoneIds": ["zone_99", "zone_100"],
        "newBounds": {
          "screen": { "x": 100, "y": 200, "width": 120, "height": 60 },
          "physical": { "x": 5.0, "y": 10.0, "width": 6.0, "height": 3.0 }
        },
        "reason": "Overlap exceeded 30% threshold"
      }
    }
    ```
...
*   **Response:**
    ...
    {
      "mergedZoneId": "zone_101",
      "label": "Expanded Spill Area",
      "type": "WET_FLOOR",
      "bounds": {
        "screen": { "x": 100, "y": 200, "width": 120, "height": 60 },
        "physical": { "x": 5.0, "y": 10.0, "width": 6.0, "height": 3.0 }
      },
      "deletedZoneIds": ["zone_99", "zone_100"],
      "expiresAt": "2026-01-26T20:00:00Z"
    }
    ```
